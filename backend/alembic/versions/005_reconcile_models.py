...
This migration uses the canonical short revision id ``005_reconcile_models``
(19 characters), the Alembic best-practice form that fits comfortably inside the
standard ``alembic_version.version_num`` column (``VARCHAR(32)``). Migration
006's ``down_revision`` references it, so the linear chain 004 -> 005 -> 006 is
preserved.
